...
module.exports = async function (deployer, network, accounts) { 
    ...
    if (supportedBlockchains.includes(network)) { 
        ...
        // Deploy Registry and Imaginovation implementation into deterministic address, same on any chain 
        const minimalImaginovationStake = web3.utils.toWei(new BN('100'), 'ether') // 100 MYST 
        const [registryAddress, ImaginovationImplementationAddress] = await deployRegistry(web3, account) 
        const registry = await Registry.at(registryAddress) 
        await registry.initialize(tokenAddress, swapRouterAddress, minimalImaginovationStake, ChannelImplementation.address, ImaginovationImplementationAddress, parentRegistry, { from: account }) 
 
        console.log('\n   > registry contract address: ', registryAddress) 
        console.log('   > Imaginovation implementation address: ', ImaginovationImplementationAddress, '\n') 
    } 
    else { 
        // Deploy WETH token 
        ...
        // Deploy Registry and Imaginovation implementation 
        const [registryAddress, ImaginovationImplementationAddress] = await deployRegistry(web3, account) 
