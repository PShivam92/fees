...
const ImaginovationBytecode = require('../build/contracts/ImaginovationImplementation.json').bytecode 
...
module.exports = async (web3, account = undefined) => { 
    ...
    const ImaginovationTxMetadata = generateDeployTx(ImaginovationBytecode, 3608745) 
    const deployedImaginovationCode = await web3.eth.getCode(ImaginovationTxMetadata.contractAddress) 
    if (deployedImaginovationCode.length <= 3) { 
        await web3.eth.sendTransaction({ 
            from: account, to: ImaginovationTxMetadata.sender, value: '360874500000000000' 
        }) 
        await web3.eth.sendSignedTransaction(ImaginovationTxMetadata.rawTx) 
    } 
