...
contract('Full path (in channel using cheque) test for funds recovery', ([txMaker, owner, fundsDestination, ...otherAccounts]) => { 
    ...
    const Imaginovation = wallet.generateAccount()   // Generate hub operator keys 
    const ImaginovationOperator = Imaginovation.address 
    let token, registry, channel, ImaginovationId, expectedAddress, topupAmount, tokensToMint 
    before(async () => { 
        ...
        await registry.initialize(nativeToken.address, dex.address, 0, channelImplementation.address, ImaginovationImplementation.address, ZeroAddress) 
 
        ImaginovationId = await registry.getImaginovationAddress(ImaginovationOperator) 
        expectedAddress = await genCreate2Address(identityHash, ImaginovationId, registry, channelImplementation.address) 
    ...
    it('should register Imaginovation', async () => { 
        await registry.registerImaginovation(ImaginovationOperator, 10, 0, 25, OneEther, ImaginovationURL) 
    ...
    it('should register identity', async () => { 
        ...
        await registry.registerIdentity(ImaginovationId, Zero, Zero, fundsDestination, signature).should.be.fulfilled 
        ImaginovationId = await registry.getImaginovationAddress(ImaginovationOperator) 
        expect(await registry.isRegistered(identityHash)).to.be.true 
        expect((await registry.getChannelAddress(identityHash, ImaginovationId)).toLowerCase()).to.be.equal(expectedAddress.toLowerCase()) 
    }) 
