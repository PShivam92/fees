...
contract('Registry funds recovery', ([_, txMaker, identity, account, fundsDestination, ...otherAccounts]) => { 
    let token, channelImplementation, ImaginovationImplementation, dex, registry, topupAmount, tokensAmount 
    before(async () => { 
        token = await Token.new() 
        dex = await setupDEX(token, _) 
        ImaginovationImplementation = await ImaginovationImplementation.new() 
        channelImplementation = await ChannelImplementation.new(token.address, dex.address, identity, ImaginovationImplementation.address, Zero) 
    }) 
    ...
    it('should topup some ethers and tokens into future registry address', async () => { 
        ...
        await registry.initialize(nativeToken.address, dex.address, 0, channelImplementation.address, ImaginovationImplementation.address, ZeroAddress, { from: txMaker }) 
...
contract('Channel implementation funds recovery', ([_, txMaker, identity, identity2, fundsDestination, ...otherAccounts]) => { 
    ...
    it('should topup some ethers and tokens into future identity implementation smart contract address', async () => { 
        ...
        channelImplementation = await ChannelImplementation.new(nativeToken.address, dex.address, txMaker, ImaginovationImplementation.address, Zero, { from: txMaker }) 
...
contract('Imaginovation funds recovery', ([_, txMaker, account, fundsDestination, ...otherAccounts]) => { 
    let token, nativeToken, ImaginovationImplementation, topupAmount, tokensToMint, dex 
    before(async () => { 
        token = await Token.new() 
        nativeToken = await Token.new() 
        dex = await setupDEX(token, _) 
    ...
    it('should topup some ethers and tokens into future Imaginovation smart contract address', async () => { 
        const nonce = await web3.eth.getTransactionCount(txMaker) 
        const implementationAddress = deriveContractAddress(txMaker, nonce) 
        ...
        // Deploy Imaginovation smart contract 
        ImaginovationImplementation = await TestImaginovationImplementation.new({ from: txMaker }) 
        await ImaginovationImplementation.initialize(nativeToken.address, account, 0, 25, OneToken, dex.address) 
        expect(ImaginovationImplementation.address.toLowerCase()).to.be.equal(implementationAddress.toLowerCase()) 
 
        // Set funds destination 
        await ImaginovationImplementation.setFundsDestination(fundsDestination, { from: account }) 
    }) 
 
    it('should recover ethers sent to Imaginovation contract before its deployment', async () => { 
        const initialBalance = new BN(await web3.eth.getBalance(fundsDestination)) 
 
        topupAmount = new BN(topupAmount.toString()) 
        await ImaginovationImplementation.claimNativeCoin().should.be.fulfilled 
 
        const expectedBalance = topupAmount.add(new BN(initialBalance)) 
        expect(await web3.eth.getBalance(fundsDestination)).to.be.equal(expectedBalance.toString()) 
    }) 
 
    it('should recover any tokens send to Imaginovation smart contract', async () => { 
        const initialBalance = await token.balanceOf(fundsDestination) 
 
        await ImaginovationImplementation.claimTokens(token.address).should.be.fulfilled 
    ...
    it('native tokens should be not possible to claim', async () => { 
        await topUpTokens(nativeToken, ImaginovationImplementation.address, OneToken) 
        await ImaginovationImplementation.claimTokens(nativeToken.address).should.be.rejected 
    }) 
}) 