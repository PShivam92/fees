...
async function genCreate2Address(identityHash, ImaginovationId, registry, implementationAddress) { 
    const byteCode = (await registry.getProxyCode(implementationAddress)) 
    const salt = web3.utils.keccak256('0x' + [identityHash.replace(/0x/, ''), ImaginovationId.replace(/0x/, '')].join('').toLowerCase()) 
...
// Generates provider's channelId in Imaginovation smart contract 
function generateChannelId(party, ImaginovationId, type = '') { 
