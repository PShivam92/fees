...
async function createConsumer(registry, identity, ImaginovationId) { 
    const channelId = await registry.getChannelAddress(identity.address, ImaginovationId) 
...
function createProvider(identity, Imaginovation) { 
    const state = { 
        invoices: { 
            // "invoiceId": { 
            //     agreementID: 1, 
...
async function createImaginovationService(Imaginovation, operator, token) { 
    ...
    this.getOutgoingChannel = async (receiver) => { 
        const channelId = await Imaginovation.getChannelId(receiver) 
 
        if (!state.channels[channelId]) { 
            state.channels[channelId] = merge({}, DEFAULT_CHANNEL_STATE, await Imaginovation.channels(channelId)) 
        } 
...
async function settlePromise(state, Imaginovation, promise) { 
    ...
    await Imaginovation.settlePromise(promise.identity, promise.amount, promise.fee, invoice.R, promise.signature) 
} 
...
function signFastWithdrawal(chainId, channelId, amount, fee, beneficiary, validUntil, nonce, identity, Imaginovation) { 
    const EXIT_PREFIX = "Exit request:" 
    ...
    const ImaginovationSignature = signMessage(message, Imaginovation.privKey) 
    expect(verifySignature(message, ImaginovationSignature, Imaginovation.pubKey)).to.be.true 
 
    return { channelId, amount, fee, beneficiary, validUntil, nonce, identitySignature, ImaginovationSignature } 
} 
...
function signChannelImaginovationReturnRequest(channelId, amount, fee, channelNonce, identity, chainId = 1) { 
    const Imaginovation_RETURN_PREFIX = "Stake return request" 
    const message = Buffer.concat([ 
        Buffer.from(Imaginovation_RETURN_PREFIX), 
        toBytes32Buffer(chainId), 
...
function signIdentityRegistration(registryAddress, ImaginovationId, stake, fee, beneficiary, identity) { 
    const message = Buffer.concat([ 
        toBytes32Buffer(1), // ChainID = 1 
        Buffer.from(registryAddress.slice(2), 'hex'), 
        Buffer.from(ImaginovationId.slice(2), 'hex'), 
...
function signConsumerChannelOpening(registryAddress, ImaginovationId, fee, identity) { 
    const message = Buffer.concat([ 
        toBytes32Buffer(1), // ChainID = 1 
        Buffer.from(registryAddress.slice(2), 'hex'), 
        Buffer.from(ImaginovationId.slice(2), 'hex'), 
...
function signUrlUpdate(registryAddress, ImaginovationId, url, nonce, identity) { 
    const message = Buffer.concat([ 
        Buffer.from(registryAddress.slice(2), 'hex'), 
        Buffer.from(ImaginovationId.slice(2), 'hex'), 
        Buffer.from(url), 
