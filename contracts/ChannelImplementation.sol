...
contract ChannelImplementation is FundsRecovery, Utils { 
    ...
    struct Imaginovation { 
        ...
        address contractAddress;   // imaginovation smart contract address, funds will be send there 
        uint256 settled;           // total amount already settled by imaginovation 
    } 
 
    Imaginovation public imaginovation; 
    ...
    */ 
    ...
    function initialize(address _token, address _dexAddress, address _identity, address _imaginovationId, uint256 _fee) public { 
        require(!isInitialized(), "Is already initialized"); 
        require(_identity != address(0), "Identity can't be zero"); 
        require(_imaginovationId != address(0), "ImaginovationID can't be zero"); 
        ...
        imaginovation = Imaginovation(IImaginovationContract(_imaginovationId).getOperator(), _imaginovationId, 0); 
    } 
    ...
    */ 
    ...
    function settlePromise(uint256 _amount, uint256 _transactorFee, bytes32 _lock, bytes memory _signature) public { 
        ...
        uint256 _unpaidAmount = _amount - imaginovation.settled; 
        ...
        imaginovation.settled = imaginovation.settled + _unpaidAmount; 
 
        // Send tokens 
        token.transfer(imaginovation.contractAddress, _unpaidAmount - _transactorFee); 
        ...
        emit PromiseSettled(imaginovation.contractAddress, _unpaidAmount, imaginovation.settled, _lock); 
    } 
 
    // Fast funds withdrawal is possible when imaginovation agrees that given amount of funds can be withdrawn 
    function fastExit(uint256 _amount, uint256 _transactorFee, address _beneficiary, uint256 _validUntil, bytes memory _operatorSignature, bytes memory _imaginovationSignature) public { 
        ...
        address _secondSigner = _msgHash.recover(_imaginovationSignature); 
        require(_secondSigner == imaginovation.operator, "Channel: have to be signed by imaginovation"); 
